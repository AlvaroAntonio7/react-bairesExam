...
const Table = (props) => {
    const valu = useSelector((selector)=> state.stock.value)
  ...
  return (
    <tr>
        ...
        <td data-testid={`${props.stock.company} - table`}>{`${props.stock.company} - table`}</td>
        <td>Rs{props.stock.price}</td>
        <td>{props.stock.quantity}</td>
        <td>Rs{props.stock.quantity*props.stock.price}</td>
        <td>Rs{currentValues*props.stock.quantity}</td>
        <td>
            <button
